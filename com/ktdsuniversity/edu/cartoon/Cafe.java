...
public class Cafe {
	...
	public void rent(String cartoonName) {
		for(int i = 0 ; i < this.books.length ; i++) {
			...
			if(eachBookName.equals(cartoonName) && !this.books[i].getIsRental()) {
				this.safe += this.books[i].getRentalFee();
				this.books[i].setIsRental(true);
			}
		}
	}
	// 내일 이어서 진행
	public void returnBook(String cartoonName) {
		for(int i = 0 ; i < this.books.length ; i ++) {
			String eachBookName = this.books[i].getName();
			if(eachBookName.equals(cartoonName) &&
				this.books[i].getIsRental()){
					this.books[i].setIsRental(false);
				}
		}
	}
}
